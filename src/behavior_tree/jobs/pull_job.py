...
class Move(object):
    ...
    def incoming(self, msg):
        ...
        if self.goal:
            ...
            for i in range( len(grounding.keys()) ):
                if grounding[str(i+1)]['primitive_action'] in ['pull']:
    ...
    def create_root(idx="1", goal=std_msgs.Empty(), controller_ns="", **kwargs):
        ...
        """
        # beahviors
        root = py_trees.composites.Sequence(name="Pull")
        blackboard = py_trees.blackboard.Blackboard()

        if goal[idx]["primitive_action"] in ['pull']:
            obj = goal[idx]['object']
        ...
        s_move15 = MovePose.MOVEP(name="Slide", controller_ns=controller_ns,
                                 action_goal={'pose': "Plan"+idx+"/place_pose"})
        s_move16 = Gripper.GOTO(name="Open", controller_ns=controller_ns,
                                action_goal=blackboard.gripper_open_pos,
                                force=blackboard.gripper_open_force)        
        s_move17 = MovePose.MOVEP(name="Detach1", controller_ns=controller_ns,
                                 action_goal={'pose': "Plan"+idx+"/place_top_pose"})
        # s_move17 = MovePose.MOVEPR(name="Detach1", controller_ns=controller_ns,
        #                          action_goal={'pose': geometry_msgs.Pose(geometry_msgs.Point(-0.05,0,0), geometry_msgs.Quaternion(0,0,0,1)),
        #                                       'frame': 'haetae_gripper_base_link'})
        # s_move18 = MovePose.MOVEPR(name="Detach2", controller_ns=controller_ns,
        #                          action_goal={'pose': geometry_msgs.Pose(geometry_msgs.Point(0,0,0.05), geometry_msgs.Quaternion(0,0,0,1)),
        #                                       'frame': 'haetae_ur5e_base_link'})
        
        pull = py_trees.composites.Sequence(name="Pull")
        pull.add_children([pose_est1, s_init1, s_move10, s_move11, s_move12, s_move13, s_move14, s_move15, s_move16, s_move17, s_init2])
