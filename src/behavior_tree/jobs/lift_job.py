...
class Move(object):
    ...
    def incoming(self, msg):
        ...
        if self.goal:
            ...
            for i in range( len(grounding.keys()) ):
                if grounding[str(i+1)]['primitive_action'] in ['lift']:
    ...
    def create_root(idx="1", goal=std_msgs.Empty(), controller_ns="", **kwargs):
        ...
        """
        # beahviors
        root = py_trees.composites.Sequence(name="Lift")
        blackboard = py_trees.blackboard.Blackboard()

        if goal[idx]["primitive_action"] in ['lift']:
            if 'object' in goal[idx].keys():
                obj = goal[idx]['object']
            elif 'obj' in goal[idx].keys():
                obj = goal[idx]['obj']
            else:
                rospy.logerr("Lift: No lift object")
                sys.exit()                
        else:
            return None
        ...
        s_move10 = MovePose.MOVEPROOT(name="Move1",
                                      controller_ns=controller_ns,
                                      action_goal={'pose': "Plan"+idx+"/grasp_top_pose"})
        s_move11 = MovePose.MOVEP(name="Move2", controller_ns=controller_ns,
                                 action_goal={'pose': "Plan"+idx+"/grasp_top_pose"})
        ...
        s_move15 = MovePose.MOVES(name="Lift", controller_ns=controller_ns,
                                 action_goal={'pose': "Plan"+idx+"/grasp_top_pose"})

        lift = py_trees.composites.Sequence(name="Lift")
        lift.add_children([pose_est1, s_init1, s_move10, s_move11, s_move12, s_move13, s_move14, s_move15])
