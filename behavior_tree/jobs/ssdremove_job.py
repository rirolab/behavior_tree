...
class Move(base_job.BaseJob):
    ...
    def incoming(self, msg):
        ...
        if self.goal:
            ...
            for i in range( len(grounding.keys()) ):
                if grounding[str(i+1)]['primitive_action'] in ['ssdremove']:
    ...
    def create_root(action_client, idx="1", goal=std_msgs.Empty(), **kwargs):
        ...
        """
        # beahviors
        root = py_trees.composites.Sequence(name="SSDReMove", memory=True)
        ...
        if goal[idx]["primitive_action"] in ['ssdremove']:
            obj         = goal[idx]['object']
        ...
        pose_est2 = WorldModel.POSE_ESTIMATOR(name="Plan"+idx,
                                              object_dict = {'target': obj,
                                                             'destination': destination}, find_loaded_loader=True, insertion=True,
        ...
        fine_tune1 = WorldModel.FINETUNE_GOALS(name="FTGoal", idx=idx, is_loaded = True)
