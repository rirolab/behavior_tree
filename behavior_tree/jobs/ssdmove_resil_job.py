...
class Move(base_job.BaseJob):
    ...
    def incoming(self, msg):
        ...
        if self.goal:
            ...
            for i in range( len(grounding.keys()) ):
                if grounding[str(i+1)]['primitive_action'] in ['ssdmoveresil']:
    ...
    def create_root(action_client, idx="1", goal=std_msgs.Empty(), **kwargs):
        ...
        """
        # beahviors
        root = py_trees.composites.Sequence(name="SSDMoveResil", memory=True)
        ...
        if goal[idx]["primitive_action"] in ['ssdmoveresil']:
            obj         = goal[idx]['object']
        ...
        selection_1.add_child(sr_move22)
        ...
        #MJ
        place.add_children([s_init6, selection_1, s_move23, s_move24, detach, s_move25])
        # place.add_children([s_init6, selection_1])
