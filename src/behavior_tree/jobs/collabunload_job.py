...
class Move(object):
    ...
    def incoming(self, msg):
        ...
        if self.goal:
            ...
            for i in range( len(grounding.keys()) ):
                if grounding[str(i+1)]['primitive_action'] in ['collabunload']:
    ...
    def create_root(idx="1", goal=std_msgs.Empty(), controller_ns="", **kwargs):
        ...
        # beahviors
        root = py_trees.composites.Sequence(name="Collabunload")
        blackboard = py_trees.blackboard.Blackboard()

        if goal[idx]["primitive_action"] in ['collabunload']:
            if 'object' in goal[idx].keys():
                obj = goal[idx]['object']
            elif 'obj' in goal[idx].keys():
                obj = goal[idx]['obj']
            else:
                rospy.logerr("CollabUNLoad: No load object")
                sys.exit()        
        ...
        sync_pose_est = WorldModel.SYNC_POSE_ESTIMATOR_HAETAE2(name="Sync"+idx, destination=destination)
        ...
        pick = py_trees.composites.Sequence(name="CollabUNLoad")
        ...
        pick.add_children([wait_condition, pose_est2, s_move20, s_move21, s_move22,s_move23,s_move24])
